...
http_receive_request( FILE *in, char *filename, size_t size )
{
    ...
    // 3 elements
    if ((split_count = string_split( requestline, ' ', &count, &sp_str)) != 3) {
...
http_send_reply( FILE *out, char *filename )
{
    ...
    else if( strcmp( ext,".html" ) == 0 )
    {
        ...
        fprintf(out, "\r\n");
        while ((count = fread(text, sizeof(char), BUFFERSIZE, fp)) > 0) {
